...
- На публичной границе аргументы представлены в UTF-8; Windows backend преобразует их в
  UTF-16.
- В версии 0.1.1-beta дочерний процесс наследует окружение родителя.
- Ненулевой код дочернего процесса является успешным результатом запуска, а не ошибкой
  фреймворка MWF.
- Невозможность создать или выполнить запрошенный образ использует
  `error_code::launch_failed`; ошибки настройки нативного контроля используют
...
- Отдельные вызовы одного stateless-экземпляра `Runner` могут выполняться конкурентно.
  Каждый вызов владеет своими нативными handles и жизненным циклом процесса.
...
MWF 0.1.1-beta не предоставляет пространства имён, контейнеры, изоляцию виртуальной
