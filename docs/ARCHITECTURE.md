...
В Windows приостановленное создание процесса применяется, когда требуется настроенная Job,
а для завершения, отмены и deadline используются нативные объекты ожидания. Запуск без
контроля пропускает настройку Job. В POSIX pipe с флагом close-on-exec позволяет отличить
ошибки до exec и ошибки `execvp` от кода завершения приложения. Запуск без контроля,
смены рабочего каталога и настройки ресурсов перед exec использует `posix_spawnp`;
контролируемое выполнение сохраняет путь через `fork` и его гарантии parent-death и группы
процессов.
