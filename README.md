...
- `Runner<ErrorModel>` для одного управляемого дочернего процесса.
...
0.1.1-beta **не** создаёт пространства имён Linux, отдельную корневую файловую систему,
...
## Измеренная производительность запуска процессов
...
В контейнере Ubuntu 24.04 с GCC 13.3 Release семь раундов по 200 запусков дали следующие
медианы на одном хосте и одном дочернем исполняемом файле:

| Сценарий | Медиана запусков/с | Изменение относительно v0.1.0 |
| --- | ---: | ---: |
| Базовый контролируемый запуск v0.1.0 | 181.34 | Базовая линия |
| Контролируемый запуск v0.1.1 | 205.35 | +13.2% |
| Только запуск v0.1.1 | 215.85 | +19.0% |
...
базовой ОС. См. [методику бенчмарка](docs/BENCHMARKS.md) и
[сравнение фреймворков](docs/COMPARISON.md).
...
- [Сравнение с процессными и sandbox-фреймворками](docs/COMPARISON.md)
